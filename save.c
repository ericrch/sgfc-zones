...
int WriteProperty(struct TreeInfo *info, struct Property *prop, FILE *sfile)
{
	...
	{
/*ERCH*/
if (prop->id == TKN_B || prop->id == TKN_W || prop->id == TKN_AB || prop->id == TKN_AW)
{
printf ("\n\n\n%s", p); 
}
		saveputc(sfile, *p);
...
{
/*printf ("%s ", v->value);*/
printf ("[%d, ", DecodePosChar(*(v->value)));
printf ("%d]\n\n", DecodePosChar(*(v->value+1)));
}
/*
if (prop->id == TKN_B || prop->id == TKN_AB)
{
GLOBAL_BOARD[DecodePosChar(*(v->value))-1][DecodePosChar(*(v->value+1))-1]=-1;
}
if (prop->id == TKN_W || prop->id == TKN_AW)
{
GLOBAL_BOARD[DecodePosChar(*(v->value))-1][DecodePosChar(*(v->value+1))-1]=1;
}*/
		}
